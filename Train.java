...
public class Train {
    ...
    public void Train(int K, int fold, ArrayList<Integer> list) throws Exception {
        int start = fold * (filesList.length / K);
        int end = start + ((filesList.length / K) - 1);
        FileWriter myWriter = new FileWriter("/home/maymuna/dbms/filename.txt");
        ...
        for (int i = 0; i < start; i++) {
            File nonMask = new File(filesList1[list.get(i)].getAbsolutePath());
            File mask = new File(filesList[list.get(i)].getAbsolutePath());
